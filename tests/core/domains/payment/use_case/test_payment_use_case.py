...
from app.persistence.model import (
    ...
    PromotionUsageCountModel, RecommendCodeModel,
)
from core.domains.payment.dto.payment_dto import PaymentUserDto, UseTicketDto, UseRecommendCodeDto
from core.domains.payment.enum.payment_enum import (
    PromotionTypeEnum,
    TicketTypeDivisionEnum, TicketSignEnum,
)
from core.domains.payment.use_case.v1.payment_use_case import (
    GetTicketUsageResultUseCase,
    UseBasicTicketUseCase, CreateRecommendCodeUseCase, GetRecommendCodeUseCase, UseRecommendCodeUseCase,
)
...
def test_use_recommend_code_when_no_recommend_code_user_then_return_success_output(
    ...
    """
    payment_provider_dto = PaymentUserDto(
        user_id=create_users[0].id
    )
    ...
    provider_user = (
        session.query(RecommendCodeModel)
            .filter(
            RecommendCodeModel.user_id == payment_provider_dto.user_id
        ).first()
    )
    ...
    receiver_user = (
        session.query(RecommendCodeModel)
            .filter(
            RecommendCodeModel.user_id == use_recommend_code_dto.user_id
        ).first()
    )
    ...
    receiver_ticket = (
        session.query(TicketModel)
            .filter(
            TicketModel.user_id == receiver_user.user_id
        ).first()
    )
    ########################################################################
...
def test_use_recommend_code_when_have_recommend_code_user_then_return_success_output(
    ...
    payment_receiver_dto = PaymentUserDto(
        user_id=create_users[1].id
    )
...
def test_use_recommend_code_when_user_already_used_code_then_return_failure_output(
    ...
    recommend_code = recommend_code_factory.build(user_id=create_users[1].id, is_used=True)
...
def test_use_recommend_code_when_code_already_been_all_used_then_return_failure_output(
    ...
    """
    # 쿠폰 제공자, 수신자 set
    recommend_code = recommend_code_factory.build(user_id=create_users[0].id, is_used=True, code_count=2)
    session.add(recommend_code)
    session.commit()
    ...
    provider_user = (
        session.query(RecommendCodeModel)
            .filter(
            RecommendCodeModel.user_id == recommend_code.user_id
        ).first()
    )
