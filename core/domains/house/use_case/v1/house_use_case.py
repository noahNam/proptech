...
class GetHousePublicDetailUseCase(HouseBaseUseCase):
    ...
    def _make_response_schema(
        ...
        public_sales = PublicSaleDetailBaseSchema(
            ...
            hhld_total_cnt=house_public_detail_entity.public_sales.hhld_total_cnt,
            park_total_cnt=house_public_detail_entity.public_sales.park_total_cnt,
            highest_floor=house_public_detail_entity.public_sales.highest_floor,
            dong_cnt=house_public_detail_entity.public_sales.dong_cnt,
...
class GetHouseMainUseCase(HouseBaseUseCase):
    ...
    def _sort_recent_public_infos(
        self, recent_public_info_entities: List[MainRecentPublicInfoEntity]
    ) -> None:
        if (
            not len(recent_public_info_entities)
            or len(recent_public_info_entities) == 1
        ):
            return None

        self._sort_public_infos_about_capital_area(
    ...
    def _sort_public_infos_image_first(
        self, recent_public_info_entities: List[MainRecentPublicInfoEntity]
    ) -> None:
        """이미지가 있는 분양 정보 우선 정렬"""
    ...
    def execute(
        ...
        # 이미지 있는 분양, 수도권 우선 정렬
        self._sort_recent_public_infos(
            recent_public_info_entities=recent_public_info_entities
        )
