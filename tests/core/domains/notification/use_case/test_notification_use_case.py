...
def test_get_notification_use_case_when_official_category_then_return_one_result(
...
def test_get_notification_use_case_when_my_category_then_return_two_result(
    session, create_users, create_notifications
):
    get_notification_dto = GetNotificationDto(
...
def test_get_badge_repo_when_my_page_then_return_true(
    create_users, create_notifications
):
    get_badge_dto = GetBadgeDto(user_id=create_users[0].id,)
...
def test_get_badge_repo_when_my_page_then_return_false(
    create_users, create_notifications
):
...
def test_update_receive_notification_settings_use_case_when_user_change_push_type_then_success(
    session, create_users
):
    dto = UpdateReceiveNotificationSettingDto(
        user_id=create_users[0].id, push_type="official", is_active=False
    )
    UpdateReceiveNotificationSettingUseCase().execute(dto=dto)

    result = GetReceiveNotificationSettingUseCase().execute(user_id=create_users[0].id)
    history_result = (
        session.query(ReceivePushTypeHistoryModel).filter_by(user_id=dto.user_id).all()
    )
...
def test_update_receive_notification_settings_to_receive_marketing_push_then_marketing_yn_is_false(
    session, create_users, app_agree_terms_factory
):
    ...
    dto = UpdateReceiveNotificationSettingDto(
        user_id=create_users[0].id, push_type="marketing", is_active=False
    )
    UpdateReceiveNotificationSettingUseCase().execute(dto=dto)

    result = GetReceiveNotificationSettingUseCase().execute(user_id=create_users[0].id)
    app_agree_terms_result = (
        session.query(AppAgreeTermsModel).filter_by(user_id=dto.user_id).first()
    )
