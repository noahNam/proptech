...
def upgrade():
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["dong_info_id"], ["dong_infos.id"], ondelete="CASCADE"
        ),
        sa.PrimaryKeyConstraint("id"),
    )
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["room_info_id"], ["room_infos.id"], ondelete="CASCADE"
        ),
        sa.PrimaryKeyConstraint("id"),
    ...
    op.add_column(
        "private_sales", sa.Column("dong_num", sa.SmallInteger(), nullable=True)
    )
    op.add_column(
        "private_sales", sa.Column("park_space_num", sa.Float(), nullable=True)
    )
    op.add_column(
        "private_sales", sa.Column("heating_type", sa.String(length=10), nullable=True)
    )
    op.add_column(
        "private_sales", sa.Column("floor_area_ratio", sa.SmallInteger(), nullable=True)
    )
    op.add_column(
        "private_sales",
        sa.Column("building_cover_ratio", sa.SmallInteger(), nullable=True),
    )
