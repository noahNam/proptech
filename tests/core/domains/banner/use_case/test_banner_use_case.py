...
def test_when_get_home_banner_use_case_then_include_present_calendar_info_by_pubsub(
    ...
    dto = GetHomeBannerDto(section_type=SectionType.HOME_SCREEN.value,
...
def test_when_get_home_banner_use_case_with_wrong_section_type_then_fail(session, create_users):
    invalid_dto = GetHomeBannerDto(section_type=SectionType.PRE_SUBSCRIPTION_INFO.value,
                                   user_id=create_users[0].id)
...
def test_when_get_pre_subscription_banner_use_case_with_wrong_section_type_then_fail(session):
