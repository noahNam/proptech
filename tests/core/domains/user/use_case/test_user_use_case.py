...
def test_upsert_user_info_when_update_is_house_owner_then_chain_update_user_data(
    ...
    # 1
    upsert_user_info_dto = UpsertUserInfoDto(
        user_id=create_users[0].id, user_profile_id=None, codes=[1005, 1006], values=["3", "202107"]
    )
    UpsertUserInfoUseCase().execute(dto=upsert_user_info_dto)
    ...
    user_infos = (
        session.query(UserInfoModel)
            .filter(UserInfoModel.user_profile_id == user_profile.id, UserInfoModel.code.in_([1005, 1006]))
            .all()
    )
...
def test_upsert_user_info_when_update_is_sub_account_then_chain_update_user_data(
    ...
    # 1
    upsert_user_info_dto = UpsertUserInfoDto(
        user_id=create_users[0].id, user_profile_id=None, codes=[1016, 1017, 1018, 1019],
    ...
    user_infos = (
        session.query(UserInfoModel)
            .filter(UserInfoModel.user_profile_id == user_profile.id, UserInfoModel.code.in_([1016, 1017, 1018, 1019]))
            .all()
    )
