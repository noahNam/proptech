...
def test_create_user_nickname_when_start_user_info_then_success(
        session
):
    UserRepository().create_user_nickname(dto=upsert_user_info_dto)
    result = session.query(UserProfileModel).filter_by(user_id=upsert_user_info_dto.user_id).first()

    assert result.nickname == upsert_user_info_dto.value
    assert result.last_update_code == upsert_user_info_dto.code


def test_update_user_nickname_when_update_user_info_then_success(
        session
):
    UserRepository().create_user_nickname(dto=upsert_user_info_dto)
...
def test_create_user_info_when_input_user_data_then_success(
        session
):
    UserRepository().create_user_info(dto=upsert_user_info_dto)

    result = session.query(UserInfoModel).filter_by(user_profile_id=upsert_user_info_dto.user_profile_id,
...
def test_update_user_info_when_input_user_data_then_success(
        session
):
    UserRepository().create_user_info(dto=upsert_user_info_dto)
...
def test_update_last_code_to_user_info_when_input_user_data_then_success(
        session
):
    UserRepository().create_user_nickname(dto=upsert_user_info_dto)
...
def test_get_user_profile_id_when_input_user_data_then_success(
        session
):
    UserRepository().create_user_nickname(dto=upsert_user_info_dto)
...
def test_get_user_profile_id_when_input_user_data_then_none(
        session
):
    UserRepository().create_user_nickname(dto=upsert_user_info_dto)
