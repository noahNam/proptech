...
from core.domains.report.schema.report_schema import (
    ...
    GetSurveysUserReportSchema, GetSurveysResultBaseSchema,
)
...
from core.domains.user.enum.user_info_enum import CodeEnum, IsHouseHolderCodeEnum, IsHouseOwnerCodeEnum, \
    IsSubAccountEnum, AssetsRealEstateEnum, AssetsCarEnum, AssetsTotalEnum
...
class GetUserReportUseCase(ReportBaseUseCase):
    def execute(
        ...
        # 유저 설문 데이터 맵핑
        user_infos: List[GetSurveysResultBaseSchema] = self._make_user_info_object(user_profile=user_profile)
    ...
    def _make_user_info_object(self, user_profile: UserProfileEntity) -> List[GetSurveysResultBaseSchema]:
        result = list()
        code_dict = {
        ...
        for user_info in user_profile.user_infos:
            if value := code_dict.get(user_info.code):
                ...
                if user_info.code == CodeEnum.ADDRESS.value or user_info.code == CodeEnum.ADDRESS_DETAIL.value:
                    ...
                    base_schema = GetSurveysResultBaseSchema(code=user_info.code, value=value)
        ...
        if address_cnt == 2:
            ...
            address_schema = GetSurveysResultBaseSchema(code=CodeEnum.ADDRESS.value, value=sido_entity.sido_name)
            address_detail_schema = GetSurveysResultBaseSchema(code=CodeEnum.ADDRESS_DETAIL.value, value=sido_entity.sigugun_name)
