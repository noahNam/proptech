...
from core.domains.house.schema.house_schema import (
    ...
    GetSearchHouseListResponseSchema, BoundingWithinRadiusResponseSchema,
)
...
class BoundingWithinRadiusPresenter:
    def transform(self, output: Union[UseCaseSuccessOutput, UseCaseFailureOutput]):
        if isinstance(output, UseCaseSuccessOutput):
            try:
                schema = BoundingWithinRadiusResponseSchema(houses=output.value)
            except ValidationError:
                return failure_response(
                    UseCaseFailureOutput(
                        type="response schema validation error",
